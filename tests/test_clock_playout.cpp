 ...
 *
 * The clocks are integer-exact. `Pace` counts phase in millionths of a
 * millisecond, so one millisecond of wall time advances a clock by `1e6 + ppm`
 * and a crystal offset of 10 ppm is exactly 10 units — no floating-point drift
 * over four simulated hours, and the same arithmetic the research document's
 ...
 *
 * Every frame and every channel of the period carries its own position, low
 * three bytes first — the width of one L24 sample, so nothing has to be added to
 * the audio for the test to be able to read it back. A period that is stored or
 * played at the wrong position, or whose channels were reordered, therefore
 * fails rather than passing as "some audio arrived".
 */
...
uint64_t position_of(const uint8_t* period) {
...
void check_clean(const std::string& what, uint64_t count,
 ...
 *
 * Channels are what a period costs in bytes and what the wire's blocks are
 * about; nothing in the clock's arithmetic depends on them, and four simulated
 * hours of the real 9216-byte period is 40 GB of copying for no extra proof. The
 * 64-channel shape is exercised where it belongs instead: on one period, in the
 * test that asserts all eight blocks share a sample position.
 */
AudioFormat sim_format(unsigned channels = 8) {
  AudioFormat format;
