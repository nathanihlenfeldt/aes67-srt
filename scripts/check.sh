...
# The phase-2 Opus shape: one block encoded, 20 ms periods. Validated here so a
# change to the codec configuration rules is caught before a Pi run.
"${BUILD_DIR}/aes67-srt" -c config/aes67-srt.opus.conf --validate
